...
import java.util.concurrent.{AbstractExecutorService, ConcurrentHashMap, ConcurrentSkipListMap, TimeUnit}
...
object Utils {
  ...
  def sideEffect[T](f: => T): Future[Unit] = {
    f
    Future.successful(())
  }

  def applyWhile(maxIteration: Int)(s: String => Boolean)(f: () => String): String = {

    def loop(value: String, count: Int): String = value match {
      case v if count >= maxIteration => v
      case v if s(v) => loop(f(), count + 1)
      case v => v
    }
...
object CircuitBreaker {
  ...
  def apply(host: String): CircuitBreaker = {
    if (!cache.containsKey(host)) {
      cache.putIfAbsent(host, new CircuitBreaker())
    }
    cache.get(host)
  }
}
